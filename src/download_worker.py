...
class DownloadWorker(QObject):
    ...
    def __init__(self, request: DownloadRequest) -> None:
        ...
        self._platform = detect_platform_type(request.url)
    ...
    def _progress_hook(self, data: dict[str, Any]) -> None:
        ...
        if state == "downloading":
            ...
            # Stall watchdog için aktivite takibi (yalnızca Kick VOD indirmelerine özeldir)
    ...
    def run(self) -> None:
        ...
        try:
            ...
            if platform == PlatformType.KICK_VIDEO and self._kick_m3u8:
    ...
    def _save_completed_record(self, platform: PlatformType, result: Any, override_target_file: Path | None = None) -> str | None:
        ...
        if is_playlist_download and isinstance(result, dict):
            ...
            first_folder: Path | None = None

            for idx, entry in enumerate(valid_entries, start=1):
                raw_path = entry.get("_filename") or entry.get("filepath")
                entry_path: Path | None = None

                if raw_path and Path(raw_path).exists() and Path(raw_path).is_file():
                    entry_path = Path(raw_path)

                if not entry_path and self.request.output_dir.exists():
                    v_id = str(entry.get("id") or "")
                    candidates = list(self.request.output_dir.rglob("*"))
                    for c in candidates:
                        if (
                            v_id
                            and v_id in c.name
                            and c.is_file()
                            and not c.name.endswith((".part", ".ytdl", ".temp"))
                        ):
                            entry_path = c
                            break
                ...
                if not first_folder:
                    first_folder = entry_path.parent

                probe = probe_media_codecs(entry_path)
                h = probe.get("height") or entry.get("height")
                ...
                total_playlist_bytes += v_rec.file_size
            ...
            summary_rec = DownloadRecord(
                ...
                media_type=self.request.media_type,
                requested_quality=self.request.quality,
                selected_height=max_height if max_height > 0 else None,
                final_path=str(summary_folder.resolve()),
                state="completed",
                file_size=total_playlist_bytes,
                completed_at=datetime.datetime.now(datetime.timezone.utc).isoformat(),
                video_codec="",
                audio_codec="",
                playlist=True,
                ...
                title=playlist_title or summary_folder.name,
                source_url=str(result.get("webpage_url") or self.request.url),
            )
            save_record(summary_rec)
            return str(summary_folder.resolve())
