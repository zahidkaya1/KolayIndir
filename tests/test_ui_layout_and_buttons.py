...
def test_button_heights_and_overflow(qapp, tmp_path, monkeypatch):
    ...
    if hasattr(MainWindow, "_start_history_validation"):
        monkeypatch.setattr(
            MainWindow,
            "_start_history_validation",
            lambda self, *args, **kwargs: None,
        )
    if hasattr(MainWindow, "_start_dependency_check"):
        monkeypatch.setattr(
            MainWindow,
            "_start_dependency_check",
            lambda self, *args, **kwargs: None,
        )
    ...
    assert paste_btn.minimumHeight() == win.analyze_button.minimumHeight()

    # Buton metinleri kırpılmaz (yeterli yükseklik)
    assert paste_btn.height() >= 40
    assert win.analyze_button.height() >= 40

    # 1366x768 alanda taşma oluşturmaz
    ...
    assert dlg.refresh_btn.height() == dlg.clear_btn.height()
    assert dlg.refresh_btn.minimumHeight() == dlg.clear_btn.minimumHeight()
    assert dlg.clear_btn.height() >= 38

    old_height = dlg.clear_btn.height()
    dlg.clear_btn.setEnabled(False)
    qapp.processEvents()
    assert dlg.clear_btn.height() == old_height

    assert dlg.minimumSizeHint().height() <= 768
    assert dlg.minimumSizeHint().width() <= 1366

    dlg.close()
    if hasattr(win, "_stop_all_threads"):
        win._stop_all_threads(for_shutdown=True)
    win.close()
    qapp.processEvents()
