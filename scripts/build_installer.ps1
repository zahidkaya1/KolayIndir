...
Write-Host "=== Loadvia 1.0.0 Installer Build Process ===" -ForegroundColor Cyan
...
if ($VerInfo.ProductName -ne "Loadvia" -or $VerInfo.FileVersion -ne "1.0.0.0") {
    Write-Error "Portable EXE version mismatch! Expected Loadvia 1.0.0.0"
    exit 1
}
...
$SetupExe = Join-Path $ReleaseDir "Loadvia-Setup-1.0.0.exe"
