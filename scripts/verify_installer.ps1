[CmdletBinding()]
param (
    [string]$SetupPath = "release\Loadvia-Setup-1.0.0.exe",
...
if ($FileVersion -ne "1.0.0.0") {
    Write-Error "Setup FileVersion is '$($VerInfo.FileVersion)', expected '1.0.0.0'"
    exit 1
}
Write-Host "[OK] Setup version metadata verified (ProductName: Loadvia, FileVersion: 1.0.0.0)" -ForegroundColor Green
